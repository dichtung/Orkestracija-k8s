...
// ovaj pristup važi jer će u kokretnom slučaju i frontend komponenta biti iza www.ws-automation.com
// a i url za api će biti www.ws-automation.com/api. U slučaju ako se koriste subdomain-i, kao npr. api.ws-automation.com
// hostname bi morao da se definiše na takav način da se na samom serveru radi zamena sa env varijablama (env substitute)
// ili da se koristi predefinisani hostname (nije dobro kad imamo više env - dev, qa, prod)
...
async function updateCat(catObj){
  await fetch(`http://${hostname}/api/v1/cat/${catObj.name}`, {
    method: 'PUT',
    headers:{
...
async function addSingleNewCat(catObj){
  await fetch(`http://${hostname}/api/v1/cat`, {
    method: 'POST',
    headers:{
      'Content-Type':'application/json'
    },
...
async function removeTheCat(catName){
  await fetch(`http://${hostname}/api/v1/cat/${catName}`, {
...
async function loadCats() {
  await fetch(`http://${hostname}/api/v1/cats`, {
    method: 'GET', // or 'PUT'
    headers: {
